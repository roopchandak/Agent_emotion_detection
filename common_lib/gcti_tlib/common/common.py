...
def SaveTestResult():
  if TestHandler():
    TestHandler().SaveTestResult()
    
def RestoreTestResult():
  if TestHandler():
    TestHandler().RestoreTestResult()
  ResetFatalErrorCnt()
...
def Error(str1 = "", str2 = "", str0 = "***", shift = 2, prtTm = 1, byServer = 0):               
  ...
  SetOption("LastErrorMessage", strErrShort)
  if InTrue(GetOption("PrintStackOnError")) or InTrue(GetOption("HTCC")):
      prtStack = 1
  else:
      prtStack = 0
  PrintError(strErr, prtStack = prtStack)
...
def SetFatalErrorCnt(str1 = "", str2 = "", str0 = "***", shift = 2, prtTm=1, prtStack=1):
  global _fatalErrorCnt
  if TestHandler():
...
ProgramWarning = ProgrammWarning # alias because of misspelling in some files
...
def testValue(value = None, expectedValue = None,  description = "", error = 0,
  ...
  if value == None and expectedValue == None: # just to produce error/warning and count it
    ...
    fldInf = ""
    bad = 0
    if expectedValue == "something" and value <> None:
      return #ok, there is something
    if type(expectedValue) == type([]):
      fldInf =  ValueMatchList(value, expectedValue, exactMatch, "")
      if fldInf:
        bad = 1
    else:
      if not ValueMatch(value, expectedValue, exactMatch):
  ...
  if bad:
    if error == 1:
      ...
      ProgramWarning("incorrect value of error = %s parameter in TestValue"%error)
  return fldInf

def checkKVListValue(value = {}, expectedValue = {}, exactMatch = 1, res = "", ignoreKeys = []):
        ...
        if not value.has_key(key):
          ...
          if expectedValue[key] == "something": #ok
            ...
            if type(expectedValueNext) == type({}):
              res = checkKVListValue(valueNext, expectedValueNext, exactMatch, res, ignoreKeys=ignoreKeys)
            elif type(expectedValueNext) == type([]):
              res = ValueMatchList(valueNext, expectedValueNext, exactMatch, res, ignoreKeys=ignoreKeys)

            else:
              if not ValueMatch(valueNext, expectedValueNext, exactMatch):
...
def testKVListValue(value = {}, expectedValue = {},  description = "", error = False, errCnt = None,
                    forceReset = -1, exactMatch = 1, ignoreKeys = [], byServer = 0):
  
  fldInf = checkKVListValue(value, expectedValue, exactMatch, ignoreKeys = ignoreKeys)
  if byServer:
    return fldInf
...
def VerifyValue(receivedValue, expectedValue,  description = "", error = 0, errCnt = None,
  ...
  """Universal method to check value for server
     ...
     error - 0, 1, 2, 3, indicates if incorrect value is error (1),serious error (2) or warning (0)
     ...
     ignoreKeys - list, keys not to take into account
  """
  if type(expectedValue) == type ({}):
    return testKVListValue(receivedValue, expectedValue,  description, error, errCnt, forceReset, exactMatch, ignoreKeys, byServer)
  else:
...
def timestamp():
   ...
   return time.strftime('%Y-%m-%d %H:%M:%S', localtime) + ',%03d' % int((now - int(now)) * 1000)

def PrintLog(obj, addTimestamp = False):
  global _logFile
  if addTimestamp:
      obj = timestamp() + " " + str(obj)
  if _printOn: 
    if _logFile: 
...
def PrintStat(obj, onScreen = 0, addTimestamp = False):
...
def with_setup(setup_function, teardown_func):
    def test_wrapper(wrappered_function):
        def wrapped(*args, **kwargs):
            try:
                setup_function()
                wrappered_function(*args, **kwargs)
            finally:
                teardown_func()
...
def ValueMatch(received, expected, exactMatch = 1):
  if exactMatch:
    return received == expected
  else:
    if type(received) == type(""):
      return Similar(received, expected)
    else:
      return received == expected


def ValueMatchList(received, expected, exactMatch=1, res="", ignoreKeys = []):
    ind = 0
    if exactMatch:
        if expected == [] and received != []:
            res = res + ("    expected: %s\n" % str(expected) +
                         "    received: %s\n" % str(received))
            return res
        for expectedElem in expected:
            try:
                received[ind]
            except Exception:
                res = res + ("    expected value :\n%s\n" % str(expectedElem) +
                             "    received value :\n%s\n" % str("MISSING ELEMENT in list"))
                return res

            if type(expectedElem) == type({}):
                res = checkKVListValue(received[ind], expectedElem, exactMatch, res, ignoreKeys=ignoreKeys)
            elif type(expectedElem) == type([]):
                res = ValueMatchList(received[ind], expectedElem, exactMatch, res, ignoreKeys=ignoreKeys)
            else:
                if not ValueMatch(received[ind], expectedElem, exactMatch):
                    res = res + ("    expected value :\n%s\n" % str(expectedElem) +
                                 "    received value :\n%s\n" % str(received[ind]))
            ind += 1
    else: #fo simple lists only:
        for expectedElem in expected:
            found = False
            if expectedElem in received:
                found = True
            else:
                for receivedElem in received:
                    if ValueMatch(receivedElem, expectedElem, exactMatch):
                        found = True
            if not found:
                res = res + ("    expected value :\n%s\n" % str(expectedElem) +
                             "    received value :\nNOT FOUND IN LIST\n")
